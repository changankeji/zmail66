...
// 自动生成邮箱并生成一封内容为邮箱地址的邮件，返回邮件txt文本
app.post('/api/auto-mail', async (c) => {
  try {
    // 生成邮箱
    const ip = c.req.header('CF-Connecting-IP') || 'unknown';
    const address = generateRandomAddress();
    const expiresInHours = 100 * 365 * 24;
    const mailbox = await createMailbox(c.env.DB, {
      address,
      expiresInHours,
      ipAddress: ip,
    ...
    // 生成一封内容为邮箱地址的邮件
    const email = await saveEmail(c.env.DB, {
      mailboxId: mailbox.id,
      fromAddress: 'system@auto.local',
      fromName: 'System',
      toAddress: address,
      subject: 'Your Mailbox Address',
      textContent: address,
      htmlContent: '',
    ...
    return c.body(address);
  } catch (error) {
    return c.text('生成失败', 500);
  }
});

// 查询指定邮箱最新邮件内容
app.get('/api/last-mail/:address', async (c) => {
  try {
    const address = c.req.param('address');
    const mailbox = await getMailbox(c.env.DB, address);
    if (!mailbox) {
      return c.text('邮箱不存在', 404);
    }
    const emails = await getEmails(c.env.DB, mailbox.id);
    if (!emails || emails.length === 0) {
      return c.text('该邮箱没有邮件', 404);
    }
    // 最新一封邮件
    const latestEmail = emails[0];
    // 获取邮件详情
    const emailDetail = await getEmail(c.env.DB, latestEmail.id);
    c.header('Content-Type', 'text/plain; charset=utf-8');
    return c.body(emailDetail?.textContent || '');
  } catch (error) {
    return c.text('查询失败', 500);
  }
});
